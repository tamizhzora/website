...
function adBanner(adCodeOrAd) {
  ...
  const type = ad ? (ad.type || "html") : "html";
  const linkUrl = ad ? (ad.linkUrl || "") : "";
  if (!code) return `<div class="ad-banner"><div class="ad-space">Your Ad Here (728×90)</div></div>`;
  if (type === "image") {
    // Image banner ads: clicking anywhere on the creative opens the
    // configured click-through link in a new tab. rel="noopener noreferrer
    // sponsored" is the responsible/safe way to link out to an advertiser —
    // it stops the opened page from reaching back into this tab and marks
    // the link as a paid/sponsored placement.
    const img = `<img src="${escapeHtml(code)}" alt="Advertisement" style="max-width:100%;display:block;margin:0 auto;border-radius:4px;">`;
    return `<div class="ad-banner">${linkUrl ? `<a href="${escapeHtml(linkUrl)}" target="_blank" rel="noopener noreferrer sponsored" aria-label="Advertisement">${img}</a>` : img}</div>`;
  }
  // "html" type — trusted admin-authored HTML/JS/markup, rendered as-is
  // (it may already contain its own link/click-through).
  ...
  const playerSection = guestGate ? `
    ...
    <div class="reel-deck">
      ...
      <div class="reel-screen-wrap" id="reelScreenWrap">
        <video id="player" preload="auto" poster="${escapeHtml(video.thumbnail)}" controlsList="nodownload noremoteplayback" autopictureinpicture oncontextmenu="return false;" playsinline>
  ...
  const body = `
    ...
    <script>
      ...
      player.addEventListener('click', () => {
        if (adActive) return;
        if (!controlsVisible()) { showReelControls(); return; }
        player.paused ? requestPlay() : player.pause();
      });
      ...
      function syncVolIcon() {
        if (reelEls.volIcon) reelEls.volIcon.parentElement.innerHTML = (player.muted || player.volume === 0) ? ICON_MUTE : ICON_VOL;
      }
      if (reelEls.volume) {
      ...
      function controlsVisible() {
        return reelEls.screenWrap.classList.contains('show-controls') || player.paused;
      }
      ...
      if (reelEls.screenWrap) {
        reelEls.screenWrap.addEventListener('mousemove', showReelControls);
      ...
      // forwards. Bug fix: a single tap on a paused/hidden-controls player
      // used to toggle play/pause immediately, so the controls (and the
      // correct play/pause icon) never got a chance to show first. Now the
      // first tap just reveals the controls; only a second tap (with
      // controls already visible) actually toggles playback — matching how
      // the play/pause button itself behaves.
      ...
      function wireTapZone(zoneEl, dir) {
        ...
        const handle = () => {
          ...
          if (now - lastTap < 320) {
            ...
            lastTap = 0;
            if (!adActive) { skip(dir); flashSeek(dir); }
          } else {
            lastTap = now;
            clearTimeout(tapTimer);
            tapTimer = setTimeout(() => {
      ...
      // Picture-in-picture.
      const ICON_PIP = '<path d="M19 7H5a2 2 0 00-2 2v6a2 2 0 002 2h4v-2H5V9h14v6h-3v2h3a2 2 0 002-2V9a2 2 0 00-2-2zm-6 6h5v4h-5z"/>';
      const ICON_PIP_ACTIVE = '<path d="M19 7H5a2 2 0 00-2 2v6a2 2 0 002 2h4v-2H5V9h14v6h-3v2h3a2 2 0 002-2V9a2 2 0 00-2-2zm-1 6h-5v4h5z"/>';
      const pipSupported = document.pictureInPictureEnabled && !player.disablePictureInPicture;
      if (reelEls.pipBtn && pipSupported) {
        ...
        player.addEventListener('enterpictureinpicture', () => {
          reelEls.pipBtn.innerHTML = '<svg viewBox="0 0 24 24">' + ICON_PIP_ACTIVE + '</svg>';
          reelEls.pipBtn.setAttribute('aria-label', 'Exit picture in picture');
        });
        player.addEventListener('leavepictureinpicture', () => {
          reelEls.pipBtn.innerHTML = '<svg viewBox="0 0 24 24">' + ICON_PIP + '</svg>';
          reelEls.pipBtn.setAttribute('aria-label', 'Picture in picture');
        });
        // Auto-enter PiP when the user leaves the tab/page while a video is
        // playing (switching tabs, minimizing, or navigating back), so
        // playback keeps going in a floating window instead of just
        // disappearing. The "autopictureinpicture" attribute on <video>
        // handles this natively in Chrome; this listener is the
        // cross-browser fallback and covers in-app back navigation too.
        const autoPipIfPlaying = () => {
          if (document.hidden && !player.paused && !adActive && !document.pictureInPictureElement) {
            player.requestPictureInPicture().catch(() => {});
          }
        };
        document.addEventListener('visibilitychange', autoPipIfPlaying);
        window.addEventListener('pagehide', autoPipIfPlaying);
      } else if (reelEls.pipBtn) {
        reelEls.pipBtn.style.display = 'none';
      }

      // Keep the floating PiP window's own play/pause/seek controls wired to
      // this player via the Media Session API, so once PiP opens it keeps
      // working across every tab and doesn't get silently dropped or cut
      // off when focus moves elsewhere.
      if ('mediaSession' in navigator) {
        try {
          navigator.mediaSession.metadata = new MediaMetadata({
            title: document.title || 'Now playing',
            artwork: player.poster ? [{ src: player.poster, sizes: '512x512', type: 'image/jpeg' }] : []
          });
        } catch (e) {}
...
async function adminDashboard(videos, users, cats, seriesList, activeTab, message, env, usearch, totalUsers, vq, vsort, txuser) {
  ...
  const adRows = ads.length ? ads.map(a => `
    <tr>
      ...
      <td>${a.priority || 0}</td>
      <td>${a.linkUrl ? `<a href="${escapeHtml(a.linkUrl)}" target="_blank" rel="noopener noreferrer" style="color:var(--gold);">🔗 link</a>` : "—"}</td>
    ...
    <div class="admin-wrap">
      ...
      ` : activeTab === "ads" ? `
        <div>
          ...
          <table class="users-table">
            <thead><tr><th>Name</th><th>Type</th><th>Placement</th><th>Priority</th><th>Link</th><th>Status</th><th>Created</th><th>Updated</th><th>Preview</th><th>Actions</th></tr></thead>
            <tbody>${adRows}</tbody>
          </table>
          <div style="margin-top:22px;max-width:560px;">
            ...
            <form method="POST" action="/admin/ads/save">
              ...
              <label>Click-through link (image ads only — opens in a new tab when clicked)</label>
              <input type="text" name="linkUrl" placeholder="https://advertiser-site.com">
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/admin/ads/save" && request.method === "POST") {
        ...
        const linkUrl = sanitizeText(form.get("linkUrl"), 500) || "";
        ...
        if (id) {
          ...
          if (idx !== -1) {
            ads[idx] = { ...ads[idx], name, type, code, linkUrl, placement, priority, startDate, endDate, updatedAt: Date.now() };
            await addAdminAuditLog(env, "admin", "AD_EDIT", name, `id=${id}`);
          }
        } else {
          ads.push({ id: crypto.randomUUID(), name, type, code, linkUrl, placement, priority, status: "active", startDate, endDate, createdAt: Date.now(), updatedAt: Date.now() });
