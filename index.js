// ============================================================================
// CLIQZORA - Complete Video Streaming Platform v3
...
function spaNavScript() {
  ...
  (function(){
    ...
    function isSpaUrl(href) {
      ...
      var p = u.pathname;
      return p === '/' || p === '/watchlater' || p === '/checkin' || p === '/claimed' || p.indexOf('/profile/') === 0;
    }
...
const SETTINGS_DEFAULTS = {
  ...
  siteName: "CLIQZORA",        // shown in header wordmark, footer, auth pages, etc.
  shortName: "",               // compact name for mobile/PWA contexts; falls back to siteName
  browserTitle: "CLIQZORA",    // global fallback <title>, combined with page-specific prefixes via brandTitle()
...
// original "CLIQZ|ORA" gold-accent split when the name is still the default).
...
function wordmarkHtml(branding) {
  const name = ((branding && branding.siteName) || "CLIQZORA").toString();
...
function brandTitle(branding, prefix) {
  const site = (branding && (branding.browserTitle || branding.siteName)) || "CLIQZORA";
...
function seoMetaTags(branding, page = {}) {
  const siteName = (branding && branding.siteName) || "CLIQZORA";
...
// Admins sometimes paste a full Drive URL (share link, "open?id=", etc.)
// instead of the bare file ID the rest of the app expects. This pulls the
// bare ID out of any of those shapes; if none match, the trimmed input is
// returned unchanged (so a real bare ID always passes straight through).
function extractDriveId(raw) {
  ...
  if (m) return m[1];
  m = v.match(/\/d\/([a-zA-Z0-9_-]+)/);
  if (m) return m[1];
  return v;
}

// ─── Google Drive streaming proxy ──────────────────────────────────────────
// Every <video>/<audio> src and every download link is a *signed* URL
// pointing back at this same worker (DRIVE_PROXY_BASE + "/?id=..&exp=..&sig=..")
// — never a raw Drive link, and no Drive credential is ever sent to the
// browser. This is what turns that signed URL into actual bytes:
//
//   1. Validate the HMAC signature + expiry using the same LINK_SECRET
//      used everywhere else in the app. That's the entire access-control
//      layer here — a locked/unpurchased video never gets a signed src
//      built for it in the first place (see the /watch route), so nothing
//      else needs checking once the signature is good.
//   2. Forward the browser's Range header (if present) to the Google Drive
//      v3 API's `alt=media` endpoint, which natively understands byte
//      ranges and returns 200/206/416 exactly like a normal file server.
//   3. Stream the upstream response body straight through to the browser —
//      it is never buffered into memory — mirroring status, Content-Type,
//      Content-Length, Content-Range and Accept-Ranges.
//
// Requirements: the Drive file must be shared "Anyone with the link"
// (Viewer), and a `GOOGLE_API_KEY` (a plain Drive-API-enabled API key —
// NOT an OAuth client, NOT a service account) must be configured on the
// worker alongside the existing DRIVE_PROXY_BASE / LINK_SECRET.
async function driveProxyResponse(request, env, url) {
  ...
  const exp = (url.searchParams.get("exp") || "").toString();
  const sig = (url.searchParams.get("sig") || "").toString();

  // Reject anything that isn't a plausible bare Drive file ID before it
  // ever touches a signature check or an outbound fetch.
  if (!id || !exp || !sig || !/^[a-zA-Z0-9_-]{10,}$/.test(id)) {
    return new Response("Bad request", { status: 400 });
  }
  if (!env.LINK_SECRET) {
    console.error("drive-proxy: LINK_SECRET is not configured on this worker");
  ...
  const expected = await hmacHex(env.LINK_SECRET, `${id}:${exp}`);
  if (expected !== sig) {
    console.error(`drive-proxy: signature mismatch for driveId=${id}`);
    return new Response("Invalid signature", { status: 403 });
  }
  if (!env.GOOGLE_API_KEY) {
    console.error("drive-proxy: GOOGLE_API_KEY is not configured on this worker");
    return new Response("Server misconfigured", { status: 500 });
  }
  ...
  const upstreamUrl = `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(id)}?alt=media&key=${encodeURIComponent(env.GOOGLE_API_KEY)}`;

  let upstream;
  try {
    upstream = await fetch(upstreamUrl, { headers: upstreamHeaders, redirect: "follow" });
  } catch (e) {
    console.error(`drive-proxy: upstream fetch threw for driveId=${id}: ${e && e.message}`);
    return new Response("Video temporarily unavailable", { status: 502 });
  }

  if (upstream.status === 404) {
    console.error(`drive-proxy: Drive file not found for driveId=${id} (deleted, or not shared "Anyone with the link")`);
    return new Response("Video not found", { status: 404 });
  }
  if (upstream.status === 403) {
    console.error(`drive-proxy: Drive returned 403 for driveId=${id} — check file sharing and GOOGLE_API_KEY restrictions`);
    return new Response("Video unavailable", { status: 502 });
  }
  if (upstream.status === 416) {
    return new Response(null, {
      status: 416,
      headers: { "Content-Range": upstream.headers.get("Content-Range") || "", "Accept-Ranges": "bytes" }
    });
  }
  if (upstream.status !== 200 && upstream.status !== 206) {
    let detail = "";
    try { detail = (await upstream.text()).slice(0, 300); } catch (e) {}
    console.error(`drive-proxy: unexpected upstream status ${upstream.status} for driveId=${id}: ${detail}`);
    return new Response("Video temporarily unavailable", { status: 502 });
  }

  // Content-Type comes straight from Drive's response, i.e. the file's
  // actual stored mimeType — never assumed to be video/mp4.
...
function loggedOutGatePage() {
  const body = `
    <header>
      <div class="wordmark">CLIQ<span>ZORA</span></div>
...
async function homePage(videos, env, sort, q, category, subCategory, session, request, priceFilter, maxCoins) {
  ...
  const userArea = session
    ? `<div class="user-menu">
        ...
        <a class="user-profile" href="/profile/${session.username}">👤 ${escapeHtml(session.name)}<span class="coin-badge" id="coinBadge">💰${session.coins}</span></a> · <a class="nav-link" href="/logout">Logout</a>
       </div>`
  ...
  const body = `
    ...
    <div class="perf"></div>
    <main>
      ${adBanner(await getAdCode(env))}
      <div class="sort-tabs">
        <a class="sort-tab ${sort !== "newest" && sort !== "views" && sort !== "purchased" ? "active" : ""}" href="/?sort=release${catParam}${qParam}${priceParam}">Release Date</a>
        <a class="sort-tab ${sort === "newest" ? "active" : ""}" href="/?sort=newest${catParam}${qParam}${priceParam}">Newest Upload</a>
    ...
    <script>
      var prefetched = window.__cliqzoraPrefetched || (window.__cliqzoraPrefetched = new Set());
...
async function watchPage(video, src, likes, dislikes, views, comments, message, related, session, adCode, userAction, locked, purchased, ratingList, seriesInfo, freeMode, guestGate, branding, alreadyDownloaded) {
  ...
  const playerSection = guestGate ? `
    ...
    <div class="player-frame">
      <video id="player" controls autoplay preload="auto" poster="${escapeHtml(video.thumbnail)}" controlsList="nodownload noremoteplayback" disablePictureInPicture oncontextmenu="return false;" src="${src}">
      </video>
    </div>
    <div class="player-controls">
      <button class="act-btn" onclick="skip(-10)">⏪ 10s</button>
      <button class="act-btn" onclick="skip(10)">10s ⏩</button>
      <select class="act-btn" id="speedSel" onchange="setSpeed(this.value)">
      ...
      <label class="act-btn" style="cursor:pointer;">
        <input type="checkbox" id="autoNextChk" checked style="margin:0;"> Auto next
      </label>
  ...
  const body = `
    ...
    <script>
      ...
      setInterval(() => { if (!player.paused) localStorage.setItem(posKey, player.currentTime); }, 5000);
...
function loginPage(message, clearLocal, returnTo, branding) {
  const body = `
    <div class="auth-wrap">
      ...
      <p style="color:var(--text-dim);font-size:13px;margin-bottom:4px;">${escapeHtml((branding && branding.siteName) || "CLIQZORA")} signs in with Google only — no passwords to remember.</p>
  ...
  const body = `
    <div class="auth-wrap">
      ...
      <form method="POST" action="/choose-username">
        ...
        <button type="submit" style="margin-top:14px;">Continue to ${escapeHtml((branding && branding.siteName) || "CLIQZORA")}</button>
      </form>
    </div>`;
...
async function adminDashboard(videos, users, cats, seriesList, activeTab, message, env, usearch, totalUsers, vq, vsort, txuser) {
  ...
  const videoRows = moviesFiltered.map((v) => `
    <tr>
      ...
      <td style="font-size:11px;white-space:nowrap;">
        ${v.hidden ? `<span class="release-pill hidden-pill">🚫 Hidden</span>` : isScheduledFuture(v) ? `<span class="release-pill" style="color:#8AB4F2;border-color:#3A5A8F;">⏰ Scheduled</span>` : `<span class="release-pill visible-pill">✓ Visible</span>`}
      </td>
      <td>
  ...
  const body = `
    <div class="admin-wrap">
      ...
      ` : activeTab === "analytics" ? (settings.liveAnalyticsEnabled === false ? `
        <div class="stat-card" style="max-width:480px;text-align:center;padding:32px 20px;">
          <div style="font-size:28px;margin-bottom:8px;">🔒</div>
          <div class="num" style="font-size:16px;">Live Analytics is turned off</div>
          <p class="admin-sub" style="margin-top:8px;">No real-time viewer counts or live data are being shown while this is off. Turn it back on in Settings to see everything again.</p>
          <button type="button" class="ban-btn" style="margin-top:10px;padding:9px 16px;font-size:12.5px;" onclick="location.href='/admin?tab=settings'">Go to Settings</button>
        </div>
      ` : `
        ...
        <div>
          ...
          <form method="POST" action="/admin/settings">
            ...
            <label style="display:flex;align-items:center;gap:8px;text-transform:none;letter-spacing:0;margin-top:14px;">
              <input type="checkbox" name="liveAnalyticsEnabled" ${settings.liveAnalyticsEnabled !== false ? "checked" : ""} style="width:auto;margin:0;"> Enable Live Analytics &amp; Real-Time Views
            </label>
            <p class="admin-sub" style="margin-top:2px;">When off, the Analytics tab and the "viewers online" badge are hidden — no live/real-time data is shown or fetched (an authenticated admin can still turn this back on here at any time to see everything again).</p>
          ...
          <div style="margin-top:32px;border-top:1px solid var(--border);padding-top:20px;">
            ...
            <p class="admin-sub">Cliqzora logs users in with Google only. Create an OAuth 2.0 Client ID in the <a href="https://console.cloud.google.com/apis/credentials" target="_blank" style="color:var(--gold);">Google Cloud Console</a> (Web application type) and set its Authorized redirect URI to <code>&lt;your-site-url&gt;/auth/google/callback</code>. Paste the credentials below.</p>
            <form method="POST" action="/admin/settings">
      ...
      ` : activeTab === "branding" ? `
        <div>
          ...
          <form method="POST" action="/admin/branding">
            ...
            <input type="text" name="siteName" maxlength="60" value="${escapeHtml(settings.siteName)}" placeholder="CLIQZORA">
            ...
            <input type="text" name="browserTitle" maxlength="80" value="${escapeHtml(settings.browserTitle)}" placeholder="CLIQZORA — Watch Movies Online">
            ...
            <input type="text" name="copyrightText" maxlength="200" value="${escapeHtml(settings.copyrightText || "")}" placeholder="© 2026 CLIQZORA. All rights reserved.">
            <label style="display:flex;align-items:center;gap:8px;text-transform:none;letter-spacing:0;">
      ...
      ` : activeTab === "add" ? `
        <div>
          ...
          <form method="POST" action="/admin/add">
            ...
            <label>Coin cost to unlock (0 = free)</label>
            <input type="number" name="coinCost" min="0" value="0">
            <label>Category</label>
      ...
      ` : activeTab === "music" ? `
        <div>
          ...
          <form method="POST" action="/admin/music-settings">
            ...
            <input type="text" name="title" value="${escapeHtml(settings.backgroundMusic.title || "")}" placeholder="e.g. Cliqzora Lounge Mix">
...
function guestHeartbeatScript() {
  ...
  <script>
  (function(){
    if (window.__cliqzoraGuestHbStarted) return;
    window.__cliqzoraGuestHbStarted = true;
    function ping() {
...
function liveUpdateScript() {
  ...
  (function(){
    ...
    if (window.__cliqzoraLiveUpdateStarted) return;
    window.__cliqzoraLiveUpdateStarted = true;
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ── Google Drive streaming proxy ──────────────────────────────────
      // Handled first, before session/ban lookups: access control for this
      // route is entirely the signed id/exp/sig query string (see
      // driveProxyResponse above), not the session cookie, and every
      // <video>/<audio> tag + download link on the site points here.
      if ((path === "/drive-proxy" || path === "/drive-proxy/") &&
          (request.method === "GET" || request.method === "HEAD" || request.method === "OPTIONS")) {
        return await driveProxyResponse(request, env, url);
      }
      ...
      if (path === "/" && request.method === "GET") {
        ...
        const html = await homePage(videos, env, sort, q, category, subCategory, session, request, priceFilter, maxCoins);
      ...
      if (path === "/watch" && request.method === "GET") {
        ...
        // But "unlocked" only ever means "no coins required" — an account is
        // still required to actually watch. A logged-out visitor is always
        // locked out of playback here, even for a free (coinCost: 0) video or
        // when free mode is on; guestGate distinguishes that case (show a
        // "Login to watch" prompt) from the normal coin-purchase gate.
        const guestGate = !session;
        ...
        return new Response(await watchPage(video, src, likeData.likes, likeData.dislikes, views, comments, msg, related, session, adCode, userAction, locked, purchased, ratingList, seriesInfo, freeMode, guestGate, settings, alreadyDownloaded), {
      ...
      if (path === "/admin/settings" && request.method === "POST") {
        ...
        const settings = {
          ...
          ...(isGoogleSection ? {} : {
            ...
            liveAnalyticsEnabled: form.get("liveAnalyticsEnabled") === "on",
      ...
      if (path === "/admin/music-settings" && request.method === "POST") {
        ...
        const settings = {
          ...
          backgroundMusic: {
            ...
            driveId: extractDriveId(form.get("driveId")),
      ...
      if (path === "/admin/add" && request.method === "POST") {
        ...
        const driveId = extractDriveId(form.get("driveId"));
        ...
        videos.unshift({
          ...
          publishAt: publishAt || null, releaseDate, hidden: false, deleted: false, createdAt: Date.now()
        });
      ...
      if (path === "/admin/edit" && request.method === "POST") {
        ...
        videos[idx] = { ...videos[idx], title, driveId, thumbnail, duration, category, subCategory, description, coinCost, seriesId: seriesId || null, part: seriesId ? part : null, partOrder, publishAt: publishAt || null, releaseDate };
        await saveVideos(env, videos);
        return Response.redirect(url.origin + "/admin?tab=videos", 303);
      }
