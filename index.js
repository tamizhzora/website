...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .ad-banner.ad-banner-image{padding:0;overflow:hidden;line-height:0;}
  .ad-banner-img{width:100%;max-height:120px;object-fit:cover;display:block;cursor:pointer;}
  ...
  .reel-icon-btn.active{color:var(--gold);}
  ...
  .reel-buffered-bar{position:absolute;left:0;bottom:-3px;height:3px;background:rgba(255,255,255,.35);border-radius:2px;pointer-events:none;width:0%;}
  .reel-tap-zone{position:absolute;top:0;bottom:52px;width:50%;z-index:3;-webkit-tap-highlight-color:transparent;}
  .reel-tap-zone.left{left:0;}
  .reel-tap-zone.right{right:0;}
  .reel-tap-flash{position:absolute;top:50%;transform:translateY(-50%);background:rgba(0,0,0,.55);color:#fff;font-size:13px;font-weight:700;padding:10px 14px;border-radius:50%;opacity:0;pointer-events:none;transition:opacity .15s ease;z-index:4;}
  .reel-tap-flash.left{left:16%;}
  .reel-tap-flash.right{right:16%;}
  .reel-tap-flash.show{opacity:1;}
  .reel-buffer-spinner{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:44px;height:44px;border:3px solid rgba(255,255,255,.25);border-top-color:var(--gold);border-radius:50%;animation:spin .9s linear infinite;display:none;z-index:4;}
  .reel-buffer-spinner.show{display:block;}
  .reel-ad-creative{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#000;cursor:default;}
  .reel-ad-clicklabel{position:absolute;bottom:34px;left:50%;transform:translateX(-50%);background:rgba(255,255,255,.14);color:#F2EDE4;font-size:11px;padding:4px 10px;border-radius:12px;pointer-events:none;}
  .reel-settings-menu{position:absolute;bottom:44px;right:8px;background:var(--bg-panel);border:1px solid var(--border);border-radius:8px;padding:10px;display:none;flex-direction:column;gap:8px;min-width:180px;z-index:6;box-shadow:0 8px 20px rgba(0,0,0,.4);}
  .reel-settings-menu.show{display:flex;}
  .reel-settings-menu label{display:flex;align-items:center;gap:8px;font-size:12.5px;color:var(--text-warm);cursor:pointer;}
  .reel-settings-menu a{font-size:12.5px;color:var(--gold);font-weight:600;text-decoration:none;}
...
const SETTINGS_DEFAULTS = {
  ...
  player: {
    ...
    adCreativeType: "none",     // "none" | "image" | "video" — what plays during the ad break
    adCreativeUrl: "",          // direct URL to the ad image/video (any publicly reachable URL)
    adClickUrl: "",             // where tapping the ad creative sends the viewer (opens in a new tab)
    keyboardShortcuts: true,    // space / arrows / M / F / P
...
// Renders a banner ad. Accepts either a raw code string (legacy callers) or
// the full ad object so an "image" ad can be a real clickable banner.
//
// Click behaviour is intentionally the plain, standard pattern: the image is
// wrapped in a normal <a target="_blank" rel="noopener noreferrer"> pointing
// at /ad-click?id=... — so it only ever opens when the visitor actually
// clicks the banner (real anchor navigation, not a script forcing a tab
// open), it opens in a new tab so they never lose the page they were on, and
// /ad-click just logs a click count server-side and 302s them on to the
// advertiser's real URL. No auto-firing, no click-hijacking, no popups.
function adBanner(adCodeOrAd) {
  const ad = typeof adCodeOrAd === "string" ? { type: "html", code: adCodeOrAd } : adCodeOrAd;
  const code = ad ? (ad.code || "") : "";
  if (!code) return `<div class="ad-banner"><div class="ad-space">Your Ad Here (728×90)</div></div>`;
  if (ad.type === "image") {
    const img = `<img class="ad-banner-img" src="${escapeHtml(code)}" alt="${escapeHtml(ad.name || "Advertisement")}" loading="lazy">`;
    const inner = ad.clickUrl && ad.id
      ? `<a href="/ad-click?id=${encodeURIComponent(ad.id)}" target="_blank" rel="noopener noreferrer" aria-label="Visit advertiser">${img}</a>`
      : img;
    return `<div class="ad-banner ad-banner-image">${inner}</div>`;
  }
  // "html" type: trusted admin-entered script/markup (e.g. an ad network tag).
  return `<div class="ad-banner">${code}</div>`;
}
...
async function homePage(videos, env, sort, q, category, subCategory, session, request, priceFilter, maxCoins, previewMode) {
  ...
  const body = `
    ...
    <div class="perf"></div>
    <main>
      ${previewMode ? "" : adBanner(await getAdForPlacement(env, "home"))}
      <div class="sort-tabs">
...
async function watchPage(video, src, likes, dislikes, views, comments, message, related, session, watchAd, userAction, locked, purchased, ratingList, seriesInfo, freeMode, guestGate, branding, alreadyDownloaded, previewMode) {
  ...
  const playerSection = guestGate ? `
    ...
    <div class="reel-deck">
      ...
      <div class="reel-screen-wrap" id="reelScreenWrap">
        <video id="player" preload="auto" poster="${escapeHtml(video.thumbnail)}" controlsList="nodownload noremoteplayback" oncontextmenu="return false;" playsinline>
          <source src="${src}" type="video/mp4">
        </video>
        <div class="reel-tap-zone left" id="reelTapLeft"></div>
        <div class="reel-tap-zone right" id="reelTapRight"></div>
        <div class="reel-tap-flash left" id="reelFlashLeft">&laquo; 10</div>
        <div class="reel-tap-flash right" id="reelFlashRight">10 &raquo;</div>
        <div class="reel-buffer-spinner" id="reelBufferSpinner"></div>
        ${pcfg.adsEnabled ? `
        <div class="reel-ad-overlay" id="reelAdOverlay">
          ...
          ${pcfg.adCreativeUrl && pcfg.adCreativeType === "video" ? `
          <video class="reel-ad-creative" id="reelAdCreative" src="${escapeHtml(pcfg.adCreativeUrl)}" muted playsinline></video>` : pcfg.adCreativeUrl ? `
          <img class="reel-ad-creative" id="reelAdCreative" src="${escapeHtml(pcfg.adCreativeUrl)}" alt="Advertisement">` : `
          ...
          ${pcfg.adClickUrl ? `<div class="reel-ad-clicklabel">Tap to visit advertiser</div>` : ""}
        ...
        <div class="reel-overlay-controls" id="reelOverlayControls">
          <div class="reel-seek-row">
            ...
            <div class="reel-buffered-bar" id="reelBufferedBar"></div>
          ...
          <div class="reel-btn-row">
            ...
            <button class="reel-icon-btn" id="reelPipBtn" aria-label="Picture in picture">
              <svg viewBox="0 0 24 24"><path d="M19 7H5a2 2 0 00-2 2v6a2 2 0 002 2h4v-2H5V9h14v6h-3v2h3a2 2 0 002-2V9a2 2 0 00-2-2zm-6 6h5v4h-5z"/></svg>
            </button>
            <button class="reel-icon-btn" id="reelSettingsBtn" aria-label="More options">
              <svg viewBox="0 0 24 24"><path d="M12 8a4 4 0 100 8 4 4 0 000-8zm8.94 4a7.99 7.99 0 00-.14-1.47l2.11-1.65-2-3.46-2.49 1a8.03 8.03 0 00-2.55-1.47L15.5 2h-4l-.37 2.95a8.03 8.03 0 00-2.55 1.47l-2.49-1-2 3.46 2.11 1.65A7.99 7.99 0 003.06 12c0 .5.05.99.14 1.47l-2.11 1.65 2 3.46 2.49-1c.76.62 1.62 1.12 2.55 1.47L8.5 22h4l.37-2.95a8.03 8.03 0 002.55-1.47l2.49 1 2-3.46-2.11-1.65c.09-.48.14-.97.14-1.47z"/></svg>
            </button>
            <button class="reel-icon-btn" id="reelFsBtn" aria-label="Fullscreen">
              <svg id="reelFsIcon" viewBox="0 0 24 24"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>
            </button>
          </div>
          <div class="reel-settings-menu" id="reelSettingsMenu">
            <label><input type="checkbox" id="reelTheatreChk" style="margin:0;width:auto;"> Theatre mode</label>
            <label><input type="checkbox" id="reelAutoNextChk" ${pcfg.autoNextDefault !== false ? "checked" : ""} style="margin:0;width:auto;"> Auto play next</label>
            ${session && !freeMode && !locked ? `<a href="#" id="reelDownloadLink" onclick="doDownload('${video.id}');return false;">⬇️ ${alreadyDownloaded ? "Download again (free)" : `Download — 💰${branding.downloadCost} coin${branding.downloadCost === 1 ? "" : "s"}`}</a>` : ""}
          </div>
        </div>
      </div>
      ${pcfg.keyboardShortcuts !== false ? `
      <div class="reel-shortcuts-hint">
        <kbd>space</kbd> play/pause &nbsp; <kbd>&larr;</kbd>/<kbd>&rarr;</kbd> seek 10s &nbsp; <kbd>&uarr;</kbd>/<kbd>&darr;</kbd> volume &nbsp; <kbd>m</kbd> mute &nbsp; <kbd>f</kbd> fullscreen &nbsp; <kbd>p</kbd> pip
      </div>` : ""}
    </div>`;
  ...
  const body = `
    ...
    <script>
      ...
      const reelEls = {
        ...
        bufferedBar: document.getElementById('reelBufferedBar'),
        bufferSpinner: document.getElementById('reelBufferSpinner'),
        ...
        fsIcon: document.getElementById('reelFsIcon'),
        pipBtn: document.getElementById('reelPipBtn'),
        settingsBtn: document.getElementById('reelSettingsBtn'),
        settingsMenu: document.getElementById('reelSettingsMenu'),
      ...
      function playAd(type, onComplete) {
        ...
        const adCreative = document.getElementById('reelAdCreative');
        if (adCreative && adCreative.tagName === 'VIDEO') { adCreative.currentTime = 0; adCreative.play().catch(() => {}); }
        ...
        function endAd() {
          ...
          if (adCreative && adCreative.tagName === 'VIDEO') adCreative.pause();
      ...
      function setReelPlayingState(playing) {
        // BUGFIX: this used to set .parentElement.innerHTML, which replaced
        // the *button's* contents with a bare <path> (destroying the <svg
        // id="reelPlayIcon"> wrapper). After the first toggle, reelEls.playIcon
        // pointed at a detached node whose .parentElement was null, so the
        // pause icon silently stopped rendering (and any toggle after that
        // threw and was swallowed). Setting the SVG's own innerHTML keeps the
        // wrapper intact so the icon updates correctly every time.
        if (reelEls.playIcon) reelEls.playIcon.innerHTML = playing ? ICON_PAUSE : ICON_PLAY;
        if (reelEls.playBtn) reelEls.playBtn.setAttribute('aria-label', playing ? 'Pause' : 'Play');
        ...
        // Whenever we pause, make sure the control bar is actually visible —
        // this is what a tap on the video is supposed to reveal.
        if (!playing) showReelControls(false);
      }
      ...
      let lastVolume = player.volume > 0 ? player.volume : 1;
      function syncVolIcon() {
        // Same bugfix as the play/pause icon: update the svg's own innerHTML,
        // not the button's, so the <svg id="reelVolIcon"> wrapper survives.
        if (reelEls.volIcon) reelEls.volIcon.innerHTML = (player.muted || player.volume === 0) ? ICON_MUTE : ICON_VOL;
      }
      if (reelEls.volume) {
        reelEls.volume.addEventListener('input', () => {
          const v = parseFloat(reelEls.volume.value);
          player.volume = v;
          player.muted = v === 0;
          if (v > 0) lastVolume = v;
          syncVolIcon();
      ...
      if (reelEls.muteBtn) reelEls.muteBtn.addEventListener('click', () => {
        ...
        if (reelEls.volume) reelEls.volume.value = player.muted ? 0 : player.volume;
        syncVolIcon();
      });
      syncVolIcon();
      ...
      function isMobileDevice() { return /Android|iPhone|iPad|iPod/i.test(navigator.userAgent) || (window.matchMedia && window.matchMedia('(pointer: coarse)').matches); }
      function enterReelFullscreen() {
        // iOS Safari doesn't support the Fullscreen API on non-video elements
        // (previous bug: calling requestFullscreen() on the wrapper silently
        // failed there) — use the video element's native fullscreen instead,
        // which also auto-rotates to landscape on its own.
        if (/iPhone|iPad|iPod/i.test(navigator.userAgent) && player.webkitEnterFullscreen) {
          player.webkitEnterFullscreen();
          return;
        }
        const req = reelEls.screenWrap.requestFullscreen || reelEls.screenWrap.webkitRequestFullscreen;
        if (req) req.call(reelEls.screenWrap);
      }
      ...
      if (reelEls.fsBtn) reelEls.fsBtn.addEventListener('click', () => {
        if (document.fullscreenElement) exitReelFullscreen();
        else enterReelFullscreen();
      });
      document.addEventListener('fullscreenchange', () => {
        const active = !!document.fullscreenElement;
        if (reelEls.fsIcon) reelEls.fsIcon.innerHTML = active ? ICON_FS_EXIT : ICON_FS_ENTER;
        // Mobile users watching in fullscreen automatically rotate to landscape.
      ...
      // Shared show/auto-hide-controls helper used by both desktop mousemove
      // and mobile tap. BUGFIX: previously mobile taps went straight to
      // toggling play/pause with no "reveal controls" step of their own —
      // the bar only ever appeared via the .reel-paused CSS rule once
      // paused, which combined with the icon bug above made the button look
      // like it "wasn't there" on the first pause. Now a tap always makes
      // sure the bar is visible.
      let reelHideTimer = null;
      function showReelControls(autoHide) {
        if (!reelEls.screenWrap) return;
      ...
      function hideReelControlsIfPlaying() {
        clearTimeout(reelHideTimer);
        if (!player.paused && reelEls.screenWrap) reelEls.screenWrap.classList.remove('show-controls');
      }
      function reelControlsVisible() {
        return !!(reelEls.screenWrap && (reelEls.screenWrap.classList.contains('show-controls') || reelEls.screenWrap.classList.contains('reel-paused')));
      }
      if (reelEls.screenWrap) {
        reelEls.screenWrap.addEventListener('mousemove', () => showReelControls(true));
        reelEls.screenWrap.addEventListener('mouseleave', hideReelControlsIfPlaying);
      }

      // Double-tap to seek 10s (mobile) — left half rewinds, right half
      // forwards. A single tap's job depends on whether the control bar is
      // already showing:
      //   • bar hidden  -> first tap just reveals the controls (play/pause,
      //     seek bar, etc.) without changing playback, same as a first tap
      //     on any standard mobile video player.
      //   • bar visible -> tap toggles play/pause (tapping again "hits" the
      //     button-equivalent area), and the bar is kept visible/refreshed
      //     so the correct play or pause icon is shown right away.
      function flashSeek(dir) {
        const el = document.getElementById(dir < 0 ? 'reelFlashLeft' : 'reelFlashRight');
        if (!el) return;
        el.classList.add('show');
        clearTimeout(el._t);
        el._t = setTimeout(() => el.classList.remove('show'), 500);
      }
      function wireTapZone(zoneEl, dir) {
        if (!zoneEl) return;
        let lastTap = 0;
        let tapTimer = null;
        const handle = () => {
          ...
          if (now - lastTap < 320) {
            ...
            lastTap = 0;
            if (!adActive) { skip(dir); flashSeek(dir); showReelControls(true); }
          } else {
            lastTap = now;
            clearTimeout(tapTimer);
            tapTimer = setTimeout(() => {
              lastTap = 0;
              if (adActive) return;
              if (!reelControlsVisible()) {
                // 1st tap: just show controls, don't touch playback.
                showReelControls(true);
              } else {
                // Controls already showing: this tap plays/pauses, and the
                // updated icon (play or pause) stays visible immediately.
                player.paused ? requestPlay() : player.pause();
                showReelControls(true);
              }
            }, 320);
        ...
        zoneEl.addEventListener('click', handle);
      }
      wireTapZone(document.getElementById('reelTapLeft'), -10);
      wireTapZone(document.getElementById('reelTapRight'), 10);

      // In-built preload/buffering system: shows how much has downloaded
      // ahead of playback, and a spinner while the player is stalled waiting
      // on data.
      function updateBufferedBar() {
        if (!reelEls.bufferedBar || !player.duration) return;
        try {
          if (player.buffered.length) {
            const end = player.buffered.end(player.buffered.length - 1);
            reelEls.bufferedBar.style.width = Math.min(100, (end / player.duration) * 100) + '%';
          }
        } catch (e) {}
      }
      player.addEventListener('progress', updateBufferedBar);
      player.addEventListener('loadedmetadata', updateBufferedBar);
      player.addEventListener('waiting', () => { if (reelEls.bufferSpinner) reelEls.bufferSpinner.classList.add('show'); });
      ['playing', 'canplay'].forEach(ev => player.addEventListener(ev, () => { if (reelEls.bufferSpinner) reelEls.bufferSpinner.classList.remove('show'); }));

      // Picture-in-picture.
      if (reelEls.pipBtn && document.pictureInPictureEnabled) {
        reelEls.pipBtn.addEventListener('click', async () => {
          try {
            if (document.pictureInPictureElement) await document.exitPictureInPicture();
            else await player.requestPictureInPicture();
          } catch (e) {}
        });
      } else if (reelEls.pipBtn) {
        reelEls.pipBtn.style.display = 'none';
      }
      // Keep the pip button's visual state (and the rest of the chrome) in
      // sync with the *actual* pip window, since it can also be opened/
      // closed by the browser's own UI, not just our button.
      player.addEventListener('enterpictureinpicture', () => {
        if (reelEls.pipBtn) reelEls.pipBtn.classList.add('active');
      });
      player.addEventListener('leavepictureinpicture', () => {
        if (reelEls.pipBtn) reelEls.pipBtn.classList.remove('active');
      });
      // Auto-PiP: if the user backgrounds/switches away from this tab while
      // the video is playing, float it into Picture-in-Picture automatically
      // instead of just silently continuing off-screen (or getting throttled
      // by the browser). Once in PiP it keeps playing across *any* tab —
      // that's native browser behaviour, nothing here pauses or stops it —
      // and it only ever closes when the person explicitly closes the PiP
      // window or comes back and exits it themselves; switching tabs again
      // never auto-cancels it.
      document.addEventListener('visibilitychange', () => {
        if (
          ...
          document.pictureInPictureElement !== player
        ) {
          player.requestPictureInPicture().catch(() => {});
        }
      });

      // Settings popover (Theatre mode / Auto-play next / Download) — every
      // control now lives inside the player itself, nothing floats outside it.
      if (reelEls.settingsBtn && reelEls.settingsMenu) {
        reelEls.settingsBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          reelEls.settingsMenu.classList.toggle('show');
        });
        document.addEventListener('click', (e) => {
          if (reelEls.settingsMenu.classList.contains('show') && !reelEls.settingsMenu.contains(e.target) && e.target !== reelEls.settingsBtn) {
            reelEls.settingsMenu.classList.remove('show');
          }
        });
      }
      const reelTheatreChk = document.getElementById('reelTheatreChk');
      if (reelTheatreChk) {
        reelTheatreChk.checked = document.body.classList.contains('theatre');
        reelTheatreChk.addEventListener('change', () => { document.body.classList.toggle('theatre', reelTheatreChk.checked); });
      }

      // Ad click-through — tapping the ad creative opens the advertiser link
      // in a new tab, without pausing or skipping the ad itself.
      const reelAdCreativeEl = document.getElementById('reelAdCreative');
      if (reelAdCreativeEl && PCFG.adClickUrl) {
        reelAdCreativeEl.style.cursor = 'pointer';
        reelAdCreativeEl.addEventListener('click', (e) => {
          e.stopPropagation();
          window.open(PCFG.adClickUrl, '_blank', 'noopener');
        });
      }
      ...
      player.addEventListener('ended', () => {
        ...
        const autoNextEl = document.getElementById('reelAutoNextChk');
        if (nextId && autoNextEl && autoNextEl.checked) {
      ...
      if (PCFG.keyboardShortcuts !== false) {
        document.addEventListener('keydown', (e) => {
          ...
          if (e.code === 'Space') {
            ...
            if (reelEls.pipBtn) reelEls.pipBtn.click();
          }
        });
      }
...
async function adminDashboard(videos, users, cats, seriesList, activeTab, message, env, usearch, totalUsers, vq, vsort, txuser) {
  ...
  const adRows = ads.length ? ads.map(a => `
    <tr>
      ...
      <td>${a.type === "image" ? (a.clicks || 0) : "—"}</td>
      <td>${new Date(a.createdAt).toLocaleDateString()}</td>
      <td>${new Date(a.updatedAt || a.createdAt).toLocaleDateString()}</td>
      <td><button type="button" class="ban-btn" style="padding:5px 10px;font-size:11px;" onclick="alert(${JSON.stringify(escapeHtml((a.code || "").slice(0, 400)) + (a.clickUrl ? "\\n\\nLinks to: " + escapeHtml(a.clickUrl) : ""))})">Preview</button></td>
  ...
  `).join("") : `<tr><td colspan="10" style="text-align:center;padding:24px;opacity:.6;">No ads yet.</td></tr>`;
    ...
    <div class="admin-wrap">
      ...
      ` : activeTab === "ads" ? `
        <div>
          ...
          <table class="users-table">
            <thead><tr><th>Name</th><th>Type</th><th>Placement</th><th>Priority</th><th>Status</th><th>Clicks</th><th>Created</th><th>Updated</th><th>Preview</th><th>Actions</th></tr></thead>
            <tbody>${adRows}</tbody>
          </table>
          <div style="margin-top:22px;max-width:560px;">
            ...
            <form method="POST" action="/admin/ads/save">
              ...
              <label>Type</label>
              <select name="type" onchange="document.getElementById('adClickUrlRow').style.display = this.value === 'image' ? '' : 'none';">
                <option value="html">HTML / Script (ad-network tag)</option>
                <option value="image">Image banner (with click-through link)</option>
              </select>
              <label>Ad code (HTML/JS, or the image URL for an Image banner)</label>
              <textarea name="code" rows="4" placeholder="&lt;script&gt;...&lt;/script&gt; or https://example.com/banner.jpg"></textarea>
              <div id="adClickUrlRow" style="display:none;">
                <label>Click-through link (Image banner only)</label>
                <input type="text" name="clickUrl" placeholder="https://advertiser-site.com/landing-page">
                <p class="admin-sub" style="margin:4px 0 0;">Opens in a new tab only when a visitor clicks the banner — never automatically.</p>
              </div>
      ...
      ` : activeTab === "player" ? `
        <div>
          ...
          <form method="POST" action="/admin/player-settings">
            ...
            <select name="adCreativeType">
              <option value="none" ${(!settings.player.adCreativeType || settings.player.adCreativeType === "none") ? "selected" : ""}>None — text slate only</option>
              <option value="image" ${settings.player.adCreativeType === "image" ? "selected" : ""}>Image</option>
              <option value="video" ${settings.player.adCreativeType === "video" ? "selected" : ""}>Video</option>
            </select>
            <label>Ad creative URL (image or video, publicly reachable)</label>
            <input type="text" name="adCreativeUrl" value="${escapeHtml(settings.player.adCreativeUrl || "")}" placeholder="https://example.com/ad-banner.jpg">
            <label>Ad click-through link</label>
            <input type="text" name="adClickUrl" value="${escapeHtml(settings.player.adClickUrl || "")}" placeholder="https://advertiser-site.com">
            <p style="font-size:12px;color:var(--text-dim);margin:-4px 0 10px;">If a creative URL is set it replaces the text slate above during ad breaks. Viewers who tap the ad creative get sent to the click-through link in a new tab — leave that blank for a non-clickable ad.</p>

            <label style="display:flex;align-items:center;gap:8px;text-transform:none;letter-spacing:0;margin-top:14px;">
              <input type="checkbox" name="keyboardShortcuts" ${settings.player.keyboardShortcuts !== false ? "checked" : ""} style="width:auto;margin:0;"> Keyboard shortcuts (space, arrows, M, F, P)
            </label>
            <label style="display:flex;align-items:center;gap:8px;text-transform:none;letter-spacing:0;">
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/watch" && request.method === "GET") {
        ...
        return new Response(await watchPage(video, src, likeData.likes, likeData.dislikes, views, comments, msg, related, session, watchAd, userAction, locked, purchased, ratingList, seriesInfo, freeMode, guestGate, settings, alreadyDownloaded, previewMode), {
      ...
      if (path === "/admin/player-settings" && request.method === "POST") {
        ...
        const settings = {
          ...
          player: {
            ...
            adCreativeType: ["none", "image", "video"].includes((form.get("adCreativeType") || "").toString()) ? form.get("adCreativeType").toString() : "none",
            adCreativeUrl: sanitizeText(form.get("adCreativeUrl"), 500) || "",
            adClickUrl: sanitizeText(form.get("adClickUrl"), 500) || "",
      ...
      if (path === "/admin/ads/save" && request.method === "POST") {
        ...
        // Click-through URL only applies to "image" banners — where the user
        // clicks, a new tab opens to this address. Ignored for "html" ads
        // since those are expected to be self-contained ad-network tags.
        const clickUrl = (form.get("clickUrl") || "").toString().trim().slice(0, 1000);
        ...
        if (id) {
          ...
          if (idx !== -1) {
            ads[idx] = { ...ads[idx], name, type, code, clickUrl, placement, priority, startDate, endDate, updatedAt: Date.now() };
            await addAdminAuditLog(env, "admin", "AD_EDIT", name, `id=${id}`);
          }
        } else {
          ads.push({ id: crypto.randomUUID(), name, type, code, clickUrl, placement, priority, status: "active", startDate, endDate, clicks: 0, createdAt: Date.now(), updatedAt: Date.now() });
      ...
      // PUBLIC: BANNER AD CLICK-THROUGH — reached only by the browser
      // actually navigating an <a target="_blank"> the visitor clicked (see
      // adBanner()). Logs a click for the admin's stats table, then 302s on
      // to the advertiser's real destination. Never fires on its own, never
      // triggered by page-load or by tapping anywhere else on the page —
      // only by a genuine click on that specific banner.
      if (path === "/ad-click" && request.method === "GET") {
        const id = (url.searchParams.get("id") || "").toString();
        const ads = await getAds(env);
        const idx = ads.findIndex(a => a.id === id);
        if (idx === -1 || ads[idx].type !== "image" || !ads[idx].clickUrl) {
          return Response.redirect(url.origin + "/", 302);
        }
        ads[idx].clicks = (ads[idx].clicks || 0) + 1;
        await saveAds(env, ads);
        let dest = ads[idx].clickUrl;
